function dx = LDM_state_eqnsta(sts, ctrl, heli, par, md,t)
    % sts  = [x, z, u, w, omg]
    % ctrl = [tht0, thtP]
    %
    ...
    udot    = -(T*sin(angR) + Dfus*cos(gma) - H*cos(angR))/heli.MF;
    wdot    = par.g - (Dfus*sin(gma) + T*cos(angR) - H*sin(angR))/heli.MF;
    ...
    % end
    omgdot  = (Q + max(0,-1.4916e+03*t+10441)-9.3127e2)/rotor.MoI;
%     omgdot  = Q/rotor.MoI; % to match decay characteristics
